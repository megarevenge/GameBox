    ...
    if (key.length() == 1 && key[0].isLetter()) {
        if (currentCol < MAX_COLS && currentRow < MAX_ROWS) {
            ...
            if (cell) {
                ...
                cell->setStyleSheet("font-size: 32px; font-weight: bold; color: black; border: 2px solid #808080;");
                currentCol++;
            }
        }
    }
...
void WordliWindow::resetGame()
{
    ...
    QMessageBox::StandardButton answer = QMessageBox::question(this,"","Restart?", QMessageBox:: No | QMessageBox::Yes);
    ...
    if(answer == QMessageBox::Yes){
        ...
        ui->usedCharsLabel->setText("Used chars: "); // edited
...
void WordliWindow::on_pushButton_2_clicked(){
